<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		if (strlen($_POST["username"]) && strlen($_POST["password"]) && strlen($_POST["name"])) {
			$result = $database->query("SELECT * FROM users WHERE account='" . addslashes($_POST["username"]) . "'");
			if ($result->num_rows) {
				echo "USER_EXISTED";
			} else {
				$database->query("INSERT INTO users (`account`, `password`, `name`) VALUES ('" . addslashes($_POST["username"]) . "', '" . md5($_POST["password"]) . "', '" . addslashes($_POST["name"]) . "')");
				echo "REG_SUCCESS";
			}
		} else {
